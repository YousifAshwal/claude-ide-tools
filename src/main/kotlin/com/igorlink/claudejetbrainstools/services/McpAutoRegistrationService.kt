...
import com.igorlink.claudejetbrainstools.util.NotificationHelper
 ...
 * 1. **Common server** (`claude-jetbrains-tools`): Handles universal operations like status, rename, and find usages
...
class McpAutoRegistrationService {
    ...
    companion object {
        ...
        const val COMMON_SERVER_NAME = "claude-jetbrains-tools"

        /** Directory name in user's home for common server installation. */
        const val COMMON_INSTALL_DIR = ".claude-jetbrains-tools"
     ...
     * 1. Extracts the common server JS file to `~/.claude-jetbrains-tools/`
     ...
     *
     * @param homeDir The user's home directory path
     * @param installDirName The directory name to create in homeDir (e.g., ".claude-jetbrains-tools")
     ...
     * - **Common server** (`claude-jetbrains-tools`): Configured with just the server path,
